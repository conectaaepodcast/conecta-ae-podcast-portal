...
import { deletePodcast, savePodcast, type ActionState } from "./actions";

type PodcastRow = Tables<"podcasts">;
...
export function PodcastForm({ initial, isAdmin }: Props) {
  const [state, formAction] = useActionState(savePodcast, {
    error: null,
  ...
  return (
    <div className="space-y-8">
      <form action={formAction} className="max-w-3xl space-y-5">
        ...
        <div>
          ...
          <input
            ...
            defaultValue={initial?.youtube_video_id ?? ""}
            className="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm"
          />
        </div>
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">
            Deixe vazio se ainda não tiver data. Conteúdo só aparece no site se estiver
            publicado e com data ≤ agora.
          </p>
        </div>
        <div>
      ...
      {isAdmin && initial?.id ? (
        <form action={deletePodcast} className="border-t border-zinc-200 pt-6">
